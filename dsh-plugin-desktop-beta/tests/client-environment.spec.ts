...
describe('advanced desktop layout', () => {
  ...
  it('owns native caption geometry with one fixed macOS drag strip above page content', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="win32"\] \.dshDesktopSidebarSurface,\s*\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="linux"\] \.dshDesktopSidebarSurface \{ grid-row: 1 \/ -1; \}/)
      expect(css).toMatch(/\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="win32"\] \.dshDesktopConversationSurface,\s*\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="win32"\] \.dshDesktopRightbarSurface,\s*\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="linux"\] \.dshDesktopConversationSurface,\s*\.dshDesktopFrame\[data-desktop-mode="advanced"\]\[data-desktop-platform="linux"\] \.dshDesktopRightbarSurface \{ grid-row: 2; \}/)
      expect(css).toMatch(/\.dshDesktopWindowsCaptionRow, \.dshDesktopLinuxCaptionRow \{[^}]*grid-column: 2 \/ -1;[^}]*grid-row: 1;/)
      expect(css).toMatch(/\.dshDesktopLinuxCaptionRow::before \{[^}]*left: env\(titlebar-area-x, 0px\);[^}]*-webkit-app-region: drag;/)
      ...
      expect(css).toContain('html:has([aria-modal="true"]) .dshDesktopWindowsCaptionRow::before,')
      expect(css).toContain('html:has([aria-modal="true"]) .dshDesktopLinuxCaptionRow::before { -webkit-app-region: no-drag !important; }')
