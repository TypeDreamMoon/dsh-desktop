...
`)
  ...
  it('projects the PowerShell profile preference into the Desktop sandbox row config', () => {
    const home = temporaryHome()
    writeFileSync(join(home, 'cordis.patch.yml'), [
      '- id: pwsh-sandbox',
      "  name: '@deepseek-ai/dsh-pwsh-sandbox'",
    ...
    // 0.1.7 keeps persisted form values in the profile's own patch layer, so the
    // preference is written where the config editor would write it.
    writeDesktopShellPreferences(home, ['pwshProfile: true'])
