...
function configFixture(overrides: Partial<DesktopSettings> = {}): ConfigFixture {
  ...
  return {
    config: {
      ...
      updateSource: field('updateSource'),
      updateChannel: field('updateChannel'),
      pwshProfile: field('pwshProfile'),
      ...GEOMETRY,
    },
...
function readConfig(value: DesktopConfig): DesktopSettings {
  return {
    ...
    updateSource: value.updateSource.get(),
    updateChannel: value.updateChannel.get(),
    pwshProfile: value.pwshProfile.get(),
  }
}
...
describe('desktop Host plugin', () => {
  ...
  it('accepts every Linux shell mode while browser access still needs compatibility mode', () => {
    ...
    // This fork implements the Linux custom-chrome window contract, so every
    // presentation mode stays available there.
    expect(() => harness.validate({ ...settings, mode: 'advanced' })).not.toThrow()
    expect(() => harness.validate({ ...settings, mode: 'extended' })).not.toThrow()
