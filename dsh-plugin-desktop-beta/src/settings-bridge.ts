...
export interface DesktopSettings {
  ...
  /**
   * Whether confined PowerShell commands load the user's profile instead of
   * running `-NoProfile`. Startup-applied: `src/profile.ts` composes it into the
   * `desktop-windows-pwsh-sandbox` row before the executor boots.
   */
  pwshProfile: boolean
}
...
export const DesktopSettingsSchema: z<DesktopSettings> = z.object({
  ...
  pwshProfile: z.boolean().default(false),
})
...
export interface DesktopShellConfig {
  ...
  /** Update source for the next generation; empty follows the packaged config. */
  updateSource: Volatile<string>
  /** Update channel for the next generation; empty follows the packaged config. */
  updateChannel: Volatile<string>
  /** Whether confined PowerShell commands load the user's profile. */
  pwshProfile: Volatile<boolean>
...
export const DesktopShellConfig = z.object({
  ...
  updateChannel: z.string().default('').volatile(),
  pwshProfile: z.boolean().default(false).volatile(),
...
export function createDesktopSettingsPort(
  ...
  const read = (): DesktopSettings => ({
    ...
    updateSource: config.updateSource.get(),
    updateChannel: config.updateChannel.get(),
    pwshProfile: config.pwshProfile.get(),
  })
...
function readCandidate(raw: unknown): DesktopSettings {
  ...
  return {
    ...
    updateSource: candidate.updateSource.get(),
    updateChannel: candidate.updateChannel.get(),
    pwshProfile: candidate.pwshProfile.get(),
  }
}
 ...
 *
 * This fork enables extended and advanced presentation on Linux, so the
 * compatibility-only rejection upstream keeps here is deliberately absent; the
 * shared `customChromeWindowOptions` builder implements the Linux window
 * contract those modes need.
 */
function assertDesktopSettings(value: DesktopSettings, _platform: NodeJS.Platform): void {
