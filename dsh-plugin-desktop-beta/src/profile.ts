...
export function prepareDesktopProfile(
  ...
  // upstream, so there is nothing left for Desktop to pin here. The Desktop Git
  // Bash preset this fork used to materialize under `~/.agent-presets/git-bash`
  // went with it: porting it back means inserting an `@deepseek-ai/dsh-agent-preset`
  // row as a profile patch instead of writing a preset directory.
