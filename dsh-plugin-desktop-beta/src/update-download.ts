...
export async function downloadDesktopUpdate(options: DownloadDesktopUpdateOptions): Promise<string> {
  ...
  let finalUrl: string
  try {
    const settled = await options.request(requestUrl, {
      method: 'GET',
...
async function resolveGithubUpdateAsset(options: {
  ...
  // The release listing reads a plain `Response`; the artifact transport wraps
  // one together with the URL its redirect chain settled on, and only the
  // installer download itself needs that settled URL.
  const request: GithubReleaseQuery['request'] =
    async (url, init) => (await options.request(url, init)).response
