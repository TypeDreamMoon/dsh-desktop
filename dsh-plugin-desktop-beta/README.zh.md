...
在所有呈现模式下，Windows PowerShell 都会保留上游 `pwsh-sandbox` 行为与 Windows ACL confinement。Launcher generation 只会把该 Host provider 替换为同一 package 中的 `dsh-plugin-desktop-beta/windows-pwsh-sandbox` 子路径。对于与上游 ACL runner 完全匹配的 argv，adapter 会让打包后的 Electron executable 通过私有 trampoline 以 Node 模式启动。Trampoline 会先精确校验上游 runner，再在导入它之前移除 Node-mode 环境变量，并确保自身这个原本没有 console 的 Windows 进程拥有一个隐藏 console。受限 PowerShell 进程随后可以继承该 console，而不必在已经使用受限 token 时自行创建。Console 分配失败会通过现有带签名的 runner 失败路径退出；全部 ACL policy 与后续失败处理仍委托给上游 runner。Desktop deploy root 还会保留 Yarn patch，在两条原生受限进程路径上把 `STARTF_USESHOWWINDOW`、现有的 `STARTF_USESTDHANDLES` 与 `SW_HIDE` 组合起来。它不会使用与上游实现不兼容的 `CREATE_NO_WINDOW` 或 `CREATE_NEW_CONSOLE` flag。直接使用 `danger-full-access` 的 PowerShell、macOS 与 Linux 执行路径保持不变；Windows confinement 失败时不会自动回退到不受限执行。在 confinement 之前，每个 Windows generation 都会自行解析 PowerShell 可执行文件：先取 `%ProgramFiles%\PowerShell\7` 标准安装，再取 `PATH` 中以规范 `PowerShell\7` 布局暴露的 PowerShell 7——其他盘上的 Program Files 会保留该布局，因此系统盘之外的安装依然优先于作为最后手段保留的 Windows PowerShell 5.1。每个候选项都是绝对路径，所以解压式 portable runtime 与 `WindowsApps` 应用执行别名都不会成为受 confinement 的可执行文件。桌面设置页的 **命令行** 会写入共享 `shell` 命名空间的 `pwshPath`——与 `settings.yaml` 接受的是同一字段——该覆盖会在下一条命令到达运行中的执行器，无需重启。同一页面的 **加载 PowerShell 配置文件** 则改为以该行的 `pwshProfile` 键重启 Host，使每条受限命令去掉上游的 `-NoProfile` 元素：命令随后会加载用户 profile，因此启动更慢，该 profile 的输出也会进入模型上下文。**生成 Git Bash 预设** 会在用户预设根目录下生成一个桌面自有的预设（id 为 `git-bash`）：复制随附的 `standard` 预设，并把其中一次性 shell 行替换为持久 Git Bash 组合——bash 方言的 `dsh-terminal-bash`（其 `shellPath` 取配置或探测到的可执行文件）加持久 `bash` 工具。Git Bash 无法在 Windows 受限令牌下启动（MSYS2 创建 signal pipe 时失败），因此该预设只在 `danger-full-access` 权限模式下可用，此时后端不做 confinement 直接启动 shell。设置页文案说明了这一点；启用了该预设却解析不到 Git Bash 时生成会 fail loud；不是本生成器创建的预设目录绝不会被替换。
