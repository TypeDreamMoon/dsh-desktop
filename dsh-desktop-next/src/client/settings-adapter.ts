...
export class NextSettingsAdapter {
  ...
  refresh = (): Promise<DesktopState> => this.reading ??= this.bridge.state().then(state => {
    if (JSON.stringify(state) !== JSON.stringify(this.current)) {
      ...
      this.shellSnapshot = this.snapshot({ mode: 'compatibility', macosMaterial: p.macosMaterial, windowsMaterial: p.windowsMaterial, linuxMaterial: p.linuxMaterial, port: p.port, openBrowser: p.browserAccess, networkExposure: p.networkExposure, logLevel: p.logLevel, updateSource: '', updateChannel: '', pwshProfile: false }, !state.busy && !state.safeMode)
