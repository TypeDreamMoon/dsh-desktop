...
export function desktopSetupWizardSelectionIsAvailable(
  ...
  _capabilities: Pick<DesktopSetupWizardInput, 'platform'>,
): boolean {
  ...
  // This fork keeps every presentation mode selectable on Linux, so the
  // compatibility-only gate upstream reinstated with the Mica removal is gone.
  return true
}
