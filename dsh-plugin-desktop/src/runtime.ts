...
export interface DesktopUpdateAdapter {
  ...
  /**
   * Download and hand one confirmed update to the platform installer from the
   * selected release source. The optional per-platform digests come from the
   * recheck that confirmed the version; the adapter applies the one matching
   * its download platform.
   */
  downloadAndOpen(
    version: string,
    signal: AbortSignal,
