...
describe('Windows Electron PowerShell sandbox adaptation', () => {
  ...
  it('carries the Desktop profile preference through the row config', () => {
    const declared = live<string | undefined>('D:\\pwsh.exe')
    const configured = { ...pwshConfig(declared), pwshProfile: true }
    const adapted = desktopWindowsPwshConfig(configured, {}, 'win32')
    expect(adapted.pwshProfile).toBe(true)
    expect(adapted.pwshPath.get()).toBe('D:\\pwsh.exe')

    const resolved = desktopWindowsPwshConfig(
      { ...pwshConfig(live<string | undefined>(undefined)), pwshProfile: true },
      {
        ProgramFiles: 'C:\\Program Files',
        SystemRoot: 'C:\\Windows',
      },
    ...
    expect(resolved.pwshPath.get()).toBe('C:\\Program Files\\PowerShell\\7\\pwsh.exe')
    expect(pwshProfileEnabled(resolved)).toBe(true)
    expect(pwshProfileEnabled(pwshConfig(live<string | undefined>(undefined)))).toBe(false)
    expect(pwshProfileEnabled({
      ...pwshConfig(live<string | undefined>(undefined)),
      pwshProfile: false,
    })).toBe(false)
  })
