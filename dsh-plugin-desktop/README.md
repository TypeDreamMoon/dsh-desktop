...
Windows PowerShell keeps the upstream `pwsh-sandbox` behavior and Windows ACL confinement in every presentation mode. The launcher generation replaces only that Host provider with the `dsh-plugin-desktop/windows-pwsh-sandbox` subpath from this same package. For the exact upstream ACL-runner argv, the adapter launches the packaged Electron executable in Node mode through a private trampoline. After validating the exact upstream runner and before importing it, the trampoline removes the Node-mode variable and ensures that its otherwise consoleless Windows process owns a hidden console. The restricted PowerShell process can then inherit that console instead of creating one while already running under the restricted token. Console allocation failure exits through the existing signed runner-failure path, and all ACL policy and subsequent failure handling remain delegated to the upstream runner. The desktop deploy root also retains its Yarn patch that combines `STARTF_USESHOWWINDOW` with the existing `STARTF_USESTDHANDLES` and `SW_HIDE` on both native restricted-process paths. It does not use the upstream-incompatible `CREATE_NO_WINDOW` or `CREATE_NEW_CONSOLE` flags. Direct `danger-full-access` PowerShell, macOS, and Linux execution are unchanged; there is no automatic unrestricted fallback when Windows confinement fails. Before confinement, each Windows generation resolves the PowerShell executable itself: the standard `%ProgramFiles%\PowerShell\7` install first, then a PowerShell 7 that `PATH` exposes in the canonical `PowerShell\7` layout — a relocated Program Files on another drive keeps that layout, so an installation outside the system drive still wins over Windows PowerShell 5.1, which stays the last resort. Every candidate is an absolute path, so an unpacked portable runtime and the `WindowsApps` App Execution Alias never become the confined executable. **Command line** on the Desktop settings page writes the shared `shell` namespace's `pwshPath` — the same field `settings.yaml` accepts — and that override reaches the running executor on the next command without a restart. **Load the PowerShell profile** on the same page instead restarts the Host with the row's `pwshProfile` key, which makes every confined command drop the upstream `-NoProfile` element: commands then load the user's profile, so they start slower and whatever that profile writes reaches the model context. **Generate the Git Bash preset** materializes a Desktop-owned preset under the user preset root (id `git-bash`) by copying the shipped `standard` preset and replacing its one-shot shell rows with a persistent Git Bash pair: `dsh-terminal-bash` in the bash dialect, whose `shellPath` is the configured or probed executable, plus the persistent `bash` tool. Git Bash cannot start under a Windows restricted token — MSYS2 fails while creating its signal pipe — so the preset only works in the `danger-full-access` permission mode, where the backend spawns the shell without confinement. The settings text states that, a requested preset without a resolvable Git Bash fails the generation loud, and a preset directory this generator did not create is never replaced.
